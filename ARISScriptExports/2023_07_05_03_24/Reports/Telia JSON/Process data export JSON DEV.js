...
writeUsingFileWriter(JSON.stringify(nested[0], 0, 4), dirPath, "processData.json")
outputFile.OutputTxt(JSON.stringify(nested[0], 0, 4));
outputFile.WriteReport(); 
 
//outputFile.OutputTxt(uniqueArray.toString(2));
//outputFile.OutputTxt(JSON.stringify(uniqueArray, 0, 4));
//outputFile.WriteReport();
...
function getDataHier(objOcc, parent, variantParent, variantDepth) {   
    ...
    //create function JSON object //HERE ADD CHECK IF THE OBJECT IS IN TREE OR MATRIX, VARIANT or PROCESS
    ...
    for (var i = 0; i < assignedModels.length; i++) {
        ...
        var tempObjOccs = assignedModels[i].ObjOccListFilter(Constants.OT_FUNC) //get functions for next level
        for (var j = 0; j < tempObjOccs.length; j++) {
            //check Hierarchy Level att
            var tempObjDef = tempObjOccs[j].ObjDef()
            
            //get connected objects to parent object
