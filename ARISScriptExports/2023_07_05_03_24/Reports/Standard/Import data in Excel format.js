...
// Status: Done (Not checked)
// Last updated: 15:44 19 Dec 007 (MSK)
// Version: 8
...
function userHasWriteRights() {
    ...
    var aUserWithGroups = [oUser].concat(oUser.UserGroups());   // AGA-3456
    for (var i = 0; i < aUserWithGroups.length; i++) {
        if ((aUserWithGroups[i].AccessRights(g_oGroup) & Constants.AR_WRITE) == Constants.AR_WRITE) {
            return true;
        }
    }
    return false;
                ...
                if (!g_oMethodFilter.IsValidAttrType(p_itemKind, p_oItem.TypeNum(), nTypeNum)) {   // BLUE-5892
                    ...
                    if (oAttr.IsValid()) {
                        ...
                        if (sAttrValue.length > 0) {                               
                            ...
                            if (bOk != true) {
                                var sError = formatstring2(getString("TEXT_14"), vAttrValue, sAttrType);
...
function getCellEntry(p_cell) {
    var cellEntry = p_cell.getCellValue();
    if (cellEntry != null) {
        return ("" + cellEntry);
    }
    return "";
