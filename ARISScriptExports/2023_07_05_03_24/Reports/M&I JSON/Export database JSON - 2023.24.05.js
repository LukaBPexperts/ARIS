...
for (var i = 0; i < selectedModels.length; i++) {
    ...
    for (var j = 0; j < textOccs.length; j++) {
        ...
        var attributeTypeNum = getAttTypeNumTextOcc(textOccs[j])

        textOccsArray.push(jSonTextLabel(content, xPos, yPos, attributeTypeNum))
    ...
    for (var j = 0; j < objectsOcc.length; j++) {
        ...
        var orientation = ""
        if (objectsOcc[j].ObjDef().TypeNum() == Constants.OT_BPMN_POOL || objectsOcc[j].ObjDef().TypeNum() == Constants.OT_BPMN_LANE) {
            var designerComp = Context.getComponent("Designer")
            var horizontalFlag = designerComp.getBPMNSupport(objectsOcc[j].Model()).isHorizontal(objectsOcc[j])
            if (horizontalFlag) {
        ...
        var fillColor = ""
        if (objectsOcc[j].ObjDef().TypeNum() == Constants.OT_STRCT_ELMT) {
            var fillColor = "#" + objectsOcc[j].getFillColor().toString(16)
        }
        ...
        var objekt = jSonObject(id, definitionId, symbolType, position, zIndex, attPlacementJsonArray, symbolTypeApiName, orientation, fillColor)
        id = null;
        definitionId = null;
        symbolType = null;
        position = null;
...
Context.writeStatus("Creating zip file!")
var zipBytes = zipDirectory(dirPath, "ArisJSON.zip");
var addFile = Context.addOutputFile("arisJSON.zip", zipBytes)

outputFile.WriteReport();

function zipDirectory(sourceDirectory, zipFileName) {
    var zipFile = new java.io.File(sourceDirectory + zipFileName);
    var zipOut = new java.util.zip.ZipOutputStream(new java.io.FileOutputStream(zipFile));
    var rootDir = new java.io.File(sourceDirectory);
    var buffer = java.lang.reflect.Array.newInstance(java.lang.Byte.TYPE, 1024);

    function addZipEntry(file, parentPath) {
        var entryPath = parentPath + file.getName();
        if (file.isDirectory()) {
            var files = file.listFiles();
            if (files.length == 0) {
                var zipEntry = new java.util.zip.ZipEntry(entryPath + "/");
                zipOut.putNextEntry(zipEntry);
                zipOut.closeEntry();
            }
            else {
                for (var i = 0; i < files.length; i++) {
                    addZipEntry(files[i], entryPath + "/");
                }
            }
        }
        else if (file.isFile() && file.getName() != zipFile.getName()) {
    ...
    for (var i = 0; i < files.length; i++) {
        var file = files[i];
        if (file.getName() != zipFile.getName()) {
            deleteFileOrDirectory(file);
        }
    }

    var zipBytes = java.nio.file.Files.readAllBytes(zipFile.toPath());
    return zipBytes;
}
...
function jSonObject(id, definitionId, symbolType, position, zIndex, attPlacementJsonArray, symbolTypeApiName, orientation, fillColor) {
...
function jSonObjTextAttValuesLang(language, languageName, value, valueHTML, valuesArray) {
...
function jSonTextLabel(content, xPos, yPos, attributeTypeNum) {
    var obj = new org.json.JSONObject();
    var contentArray = new org.json.JSONArray(content);
    obj.put("content", contentArray);
    obj.put("xPos", xPos);
    obj.put("yPos", yPos);
    obj.put("attributeTypeNum", attributeTypeNum);
...
function getContentLang(textOcc) {
    ...
    for (var i = 0; i < allLocales.length; i++) {
        ...
        if (value != "") {
            var valueHTML = textOcc.TextDef().Attribute(Constants.AT_NAME, allLocales[i]).getHTMLFormattedValue().getHTML()
            contentArray = jSonObjTextAttValuesLang(language, languageName, value, valueHTML, contentArray);
        }
    }
    return contentArray.toString(2)
}

function getAttTypeNumTextOcc(textOcc) {
    var attributeType = ""
    var modelAttrOccList = textOcc.AttrOccList();
    if (modelAttrOccList.length > 0) {
        var attrNo = modelAttrOccList[0].AttrDef(-1).getValue()
        if (parseInt(attrNo) > 0) {
            attributeType = attrNo
        }
    }
