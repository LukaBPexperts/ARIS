...
// Datenstrukturen fuer Objekt-, Attribut-, Symbol- und Kantentypen
__usertype_tobjecttype = function() {
  this.ntypenum = 0;
  this.sname = "";
}

__usertype_tattributetype = function() {
  this.ntypenum = 0;
  this.sname = "";
}

__usertype_tsymboltype = function() {
  this.ntypenum = 0;
  this.sname = "";
}

__usertype_tconnectiontype = function() {
  this.ntypenum = 0;
  this.sname = "";
}
...
__usertype_tobjecttypemappingentry = function() {
  this.sourceobjecttype = new __usertype_tobjecttype();
  this.targetobjecttype = new __usertype_tobjecttype();
  this.nattributefiltertypenum = 0;
// 0 falls Filter nicht gesetzt
  this.sattributefiltername = "";
// Leerstring falls Filter nicht gesetzt
}
...
__usertype_tobjecttypemappingsymbolentry = function() {
  this.sourceobjecttype = new __usertype_tobjecttype();
  this.sourceobjectsymboltype = new __usertype_tsymboltype();
  this.targetobjecttype = new __usertype_tobjecttype();
}
...
__usertype_tobjectandsymboltypemappingentry = function() {
  ...
  this.sourceobjectsymboltype = new __usertype_tsymboltype();
  this.targetobjecttype = new __usertype_tobjecttype();
  this.targetobjectsymboltype = new __usertype_tsymboltype();
}
...
__usertype_tobjecttypemappingconnectiontypeentry = function() {
  this.sourceobjecttype1 = new __usertype_tobjecttype();
  this.sourceconnectiontype = new __usertype_tconnectiontype();
  this.sourceobjecttype2 = new __usertype_tobjecttype();
  this.targetobjecttype1 = new __usertype_tobjecttype();
  this.targetobjecttype2 = new __usertype_tobjecttype();
}
...
__usertype_tconnectiontypemappingentry = function() {
  ...
  this.sourceobjecttype2 = new __usertype_tobjecttype();
  this.targetobjecttype1 = new __usertype_tobjecttype();
  this.targetconnectiontype = new __usertype_tconnectiontype();
  this.targetobjecttype2 = new __usertype_tobjecttype();
}
...
function performchanges(objecttypemapping, objectandsymboltypemapping, connectiontypemapping, tErrorTables)
{
  ...
  var oobjdefs = null;   var oobjdef = null;   var ocopyofobjdef = null;
  
  var aOTNs = getOTNsFromObjTypeMap();     // BLUE-5456
  oobjdefs = ArisData.getActiveDatabase().Find(Constants.SEARCH_OBJDEF, aOTNs);
  ...
  var objCount = 0;
  for ( i = 0 ; i < oobjdefs.length ; i++ ){
    oobjdef = oobjdefs[i];
    for ( j = 0 ; j < objecttypemapping.length; j++ ){
      if (oobjdef.TypeNum() == objecttypemapping[j].sourceobjecttype.ntypenum) {        // Objekttype soll umgesetzt werden
        // Attributfilter berücksichtigen
        if (objecttypemapping[j].nattributefiltertypenum == 0 || oobjdef.Attribute(objecttypemapping[j].nattributefiltertypenum, g_nloc).IsMaintained()) {
          ...
          if (targetsymboltype.ntypenum != 0) {
            ocopyofobjdef = copyobject(oobjdef, objecttypemapping[j], notcopiedattributes, notcopiedlinks);
  ...
  var oobjoccs = null;   var oobjocc = null;   var ocopyofobjocc = null; 

  var occCount = 0;
  for ( i = 0 ; i < mappedobjects.length; i++ ){
    ...
    if (oobjdef.IsValid() && ocopyofobjdef.IsValid()) {
      oobjoccs = oobjdef.OccList();
      for ( j = 0 ; j < oobjoccs.length ; j++ ){
        oobjocc = oobjoccs[j];
        ...
        if (targetsymboltype.ntypenum != 0) {
          ocopyofobjocc = copyobjectoccurence(oobjocc, ocopyofobjdef, targetsymboltype);
  ...
  var opartnerobjocc = null;   var ocxndef = null;   var ocopyofcxndef = null;   var ocxnoccs = null;   var ocxnocc = null;   var ocopyofcxnocc = null;   var osrcobjocc = null;   var otrgobjocc = null; 
  var targetconnectiontype = new __usertype_tconnectiontype(); 
  var bpartneristarget = false; 
  
  var setOfDoneCxnOccs = new java.util.HashSet();

  var cxnCount = 0;
  for ( i = 0 ; i < mappedobjectoccurences.length ; i++ ){
    ...
    for ( j = 0 ; j < ocxnoccs.length ; j++ ){
      ...
      // Partner ermitteln
      opartnerobjocc = ocxnocc.SourceObjOcc();
      bpartneristarget = false;
      ...
      // Partner umgesetzt ?
      for ( k = 0 ; k < mappedobjectoccurences.length; k++ ){
      ...
      ocxndef = ocxnocc.CxnDef();
      ocopyofcxndef = null;

      for ( k = 0 ; k < mappedconnections.length; k++ ){
      ...
      if (ocopyofcxndef == null || !ocopyofcxndef.IsValid()) {
        targetconnectiontype = gettargetconnectiontypeforobjectpairandconnection(nobjtypenum, npartnerobjtypenum, ncxntypenum, connectiontypemapping);

        if (targetconnectiontype.ntypenum != 0) {
          ocopyofcxnocc = ocxnocc.Model().CreateCxnOcc(osrcobjocc, otrgobjocc, targetconnectiontype.ntypenum, ocxnocc.PointList(), false, true);
  ...
  var delCount = 0;
  for ( i = 0 ; i < objdefguidstodelete.value.length ; i++ ){
    oobjdef = ArisData.getActiveDatabase().FindGUID(objdefguidstodelete.value[i]);

    if (oobjdef.IsValid()) {
      var bExemptFromDelete = false;
      for( j = 0 ; j < objdefguidstoexemptfromdelete.value.length ; j++ ){
  ...
  // BLUE-5456
  function getOTNsFromObjTypeMap() {
    var typeSet = new java.util.HashSet();
    for (var i = 0; i < objecttypemapping.length; i++) {
        typeSet.add(objecttypemapping[i].sourceobjecttype.ntypenum);
    }
    return [].concat(typeSet.toArray());      
  }
}
...
function copycxnattributes(ocxndef, ocopyofcxndef)
{
  ...
  for (var i = 0 ; i < ArisData.getActiveDatabase().LanguageList().length ; i++ ) {
    var olanguage = ArisData.getActiveDatabase().LanguageList()[i];
...
{
  var __functionResult = new __usertype_tconnectiontype();

  var i = 0; 

  for ( i = 0 ; i < connectiontypemapping.value.length; i++ ){
    // Anubis 436328
...
{
  var __functionResult = new __usertype_tsymboltype();

  var i = 0;   var j = 0;   var idxwithsymbolmapping = 0; 
  var bfoundsymbolmapping = false; 

  for ( i = 0 ; i < objectandsymboltypemapping.value.length; i++ ){
    if (objectandsymboltypemapping.value[i].sourceobjecttype.ntypenum == nobjecttypenum) {
      if (objectandsymboltypemapping.value[i].sourceobjectsymboltype.ntypenum == 0) {
        ...
        for ( j = i ; j < objectandsymboltypemapping.value.length; j++ ){
...
function copyobject(oobjdef, mappingentry, notcopiedattributes, notcopiedlinks)
{
  var __functionResult = null;

  var ofilter = ArisData.getActiveDatabase().ActiveFilter();

  var ocopyofobjdef = null; 
  ocopyofobjdef = oobjdef.Group().CreateObjDef(mappingentry.targetobjecttype.ntypenum, oobjdef.Name(g_nloc), g_nloc);

  var i = 0;   var j = 0;   var k = 0;   var nloc = 0; 
  var olanguage = null;   var oattrlist = null;   var oattr = null;   var ocopyofattr = null;   var oassignedmodels = null;   var omodel = null; 

  var bdonotshownotcopiedattribute = false; 

  var langList = ArisData.getActiveDatabase().LanguageList();
  for ( i = 0 ; i < langList.length ; i++ ){
    olanguage = langList[i];
    nloc = olanguage.LocaleId();
    oattrlist = oobjdef.AttrList(nloc);

    for ( j = 0 ; j < oattrlist.length ; j++ ){
      oattr = oattrlist[j];
      if (oattr.IsValid()) {
        ocopyofattr = ocopyofobjdef.Attribute(oattr.TypeNum(), nloc);
        if (ocopyofattr.IsValid()) {
          switch(ofilter.AttrBaseType(oattr.TypeNum())) {
            case Constants.ABT_BOOL:
            case Constants.ABT_COMBINED:
  ...
  oassignedmodels = oobjdef.AssignedModels();
  for ( i = 0 ; i < oassignedmodels.length ; i++ ){
    omodel = oassignedmodels[i];
  ...
  __functionResult = ocopyofobjdef;
  return __functionResult;
}
...
{
  var __functionResult = new __usertype_tmappedobject();
...
var g_dlg1_attributetypes    = new Array();    // __usertype_tattributetype()
...
function showchangeobjecttypedialog1(objecttypemapping)
{
  ...
  var userdialog = Dialogs.createNewDialogTemplate(660, 406, getString("TEXT4"), "dlgfuncchangeobjecttype1");
  // %GRID:10,7,1,1
  ...
  userdialog.CheckBox(20, 173, 500, 15, getString("TEXT8"), "chkAttributFilter");
//  userdialog.Text(40, 147, 260, 14, getString("TEXT9"));
  userdialog.DropListBox(80, 190, 220, 70, attributetypenames, "lstAttributes");
  userdialog.Text(20, 192, 50, 14, getString("TEXT10"));
  userdialog.PushButton(20, 220, 150, 21, getString("TEXT11"), "butAdd");
  userdialog.Text(20, 250, 620, 14, getString("TEXT12"));
  userdialog.ListBox(20, 266, 620, 91, mappingtexts, "lstMappings");
  userdialog.PushButton(20, 360, 150, 21, getString("TEXT13"), "butRemove");
  userdialog.OKButton();
...
//  userdialog.HelpButton("HID_5df77ad0_7500_11d9_768f_a722316b722b_dlg_01.hlp");
  ...
  var sSection = "SCRIPT_5df77ad0_7500_11d9_768f_a722316b722b";  
  ReadSettingsDlgValue(dlg, sSection, "chkAttributFilter", 0);
  ReadSettingsDlgValue(dlg, sSection, "lstAttributes", 0);

  var nuserchoice = Dialogs.show( __currentDialog = dlg);  // Dialog anzeigen

  // Write dialog settings to config  
  if (nuserchoice != 0) {
      WriteSettingsDlgValue(dlg, sSection, "chkAttributFilter");
      WriteSettingsDlgValue(dlg, sSection, "lstAttributes");
  }      
  ...
  switch(action) {
    case 1:
      ...
      if (__currentDialog.getDlgValue("chkAttributFilter") == 0) {
        __currentDialog.setDlgEnable("lstAttributes", false);
      } else {
        __currentDialog.setDlgEnable("lstAttributes", true);
      }
    ...
    case 2:
      ...
      switch(dlgitem) {
        ...
        case "butAdd":
          ...
          if (idxsource != -1 && idxtarget != -1) {
            ...
            if (g_dlg1_sourceobjecttypes[idxsource].ntypenum != g_dlg1_targetobjecttypes[idxtarget].ntypenum) {
              if (__currentDialog.getDlgValue("chkAttributFilter") != 0) {
                idxattribfilter = __currentDialog.getDlgValue("lstAttributes");
                nattribtypenum = g_dlg1_attributetypes[idxattribfilter].ntypenum;
                // Mapping mit Attributfilter hinzufuegen
                addobjecttypemappingwithattributefilter(g_dlg1_sourceobjecttypes[idxsource], g_dlg1_targetobjecttypes[idxtarget], nattribtypenum, g_dlg1_objecttypemapping);
              } else {
                // Mapping hinzufuegen
              ...
              // Quellobjekttyp entfernen
              g_dlg1_sourceobjecttypes = removeobjecttype(g_dlg1_sourceobjecttypes, idxsource);

              // Zielobjekttyp entfernen
              //g_dlg1_targetobjecttypes = removeobjecttype(g_dlg1_targetobjecttypes, idxtarget);              

              if (g_dlg1_sourceobjecttypes.length > 0) {
                sourceobjectnames = getobjecttypenames(g_dlg1_sourceobjecttypes);
              }

              __currentDialog.setDlgListBoxArray("lstSourceObjects", sourceobjectnames);

              if (g_dlg1_targetobjecttypes.length > 0) {
                targetobjectnames = getobjecttypenames(g_dlg1_targetobjecttypes);
              }
              __currentDialog.setDlgListBoxArray("lstTargetObjects", targetobjectnames);
        ...
        case "butRemove":
          idxmapping = __currentDialog.getDlgValue("lstMappings");
          if (idxmapping != -1) {
          // Mapping selektiert
            sourceobjecttype = g_dlg1_objecttypemapping[idxmapping].sourceobjecttype;
            targetobjecttype = g_dlg1_objecttypemapping[idxmapping].targetobjecttype;

            g_dlg1_objecttypemapping = removeobjecttypemapping(g_dlg1_objecttypemapping, idxmapping);
            // Mapping entfernen
            ...
            // Quellobjekttyp hinzufuegen
            addobjecttype(sourceobjecttype, g_dlg1_sourceobjecttypes);

            // Zielobjekttyp hinzufuegen
            //addobjecttype(targetobjecttype, g_dlg1_targetobjecttypes);
        ...
        case "lstMappings":
          idxmapping = __currentDialog.getDlgValue("lstMappings");
          __currentDialog.setDlgEnable("butRemove", idxmapping != -1);
        break;
...
var g_dlg2_targetobjecttype             = new __usertype_tobjecttype(); 
...
function buildobjecttypemappingsymbolsfromobjecttypemapping(objecttypemapping)
{
  ...
  var i = 0;   var j = 0;   var numobjecttypemappingsymbols = 0;   var offset = 0; 
  var sourceobjectsymbols = new Array(); 
  var sourceobjecttype = new __usertype_tobjecttype(); 

  numobjecttypemappingsymbols = 0;

  for ( i = 0 ; i < objecttypemapping.length; i++ ){
    ...
    if (sourceobjectsymbols.length > 0) {
      ...
      for ( j = 0 ; j < sourceobjectsymbols.length; j++ ){
...
{
  var i = 0;   var idxtoinsert = 0; 

  idxtoinsert = -1;

  if (objecttypemappingsymbols.length > 0) {
    for ( i = 0 ; i < objecttypemappingsymbols.length ; i++ ){
  ...
  if (sourceobjectsymboltype.ntypenum == 0) {
    ...
    if (sourceobjectsymbols.length > 0) {
      ...
      for(i = objecttypemappingsymbols.length-1; i>idxtoinsert+numtoadd-1; i--) {
      ...
      // Eintraege fuer restliche Symboltypen anlegen
      for ( i = 0 ; i < sourceobjectsymbols.length ; i++ ){
    ...
    // Eintrag fuer "Alle Symboltypen" anlegen
    for(i = objecttypemappingsymbols.length; i>idxtoinsert; i--) {
...
function removeobjecttypemappingsymbol(sourceobjecttype, sourceobjectsymboltype, idxmapping, objecttypemappingsymbols)
{
  ...
  var i = 0;   var startidxtoremove = 0;   var endidxtoremove = 0;   var numremoved = 0; 
  ...
  if (bremoveall) {
    ...
    for ( i=(idxmapping + 1) ; i < objecttypemappingsymbols.length; i++ ){
      if (objecttypemappingsymbols[i].sourceobjecttype.ntypenum == sourceobjecttype.ntypenum) {
        endidxtoremove = i;
      } else {
        break;
  ...
  for ( i=(endidxtoremove + 1) ; i < objecttypemappingsymbols.length; i++ ){
...
{
  var i = 0;   var startidxtoremove = 0;   var endidxtoremove = 0;   var numtoremove = 0;   var idxtoinsert = 0; 

  if (sourceobjectsymboltype.ntypenum == 0) {
    // ALLE SYMBOLTYPEN -> bestehende entfernen
    startidxtoremove = -1;
    for ( i = 0 ; i < objectandsymboltypemapping.length; i++ ){
    ...
    if (startidxtoremove > - 1) {
      ...
      for ( i = (startidxtoremove + 1) ; i < objectandsymboltypemapping.length; i++ ){
        if (objectandsymboltypemapping[i].sourceobjecttype.ntypenum == sourceobjecttype.ntypenum) {
          endidxtoremove = i;
        } else {
          break;
      ...
      for ( i=(endidxtoremove + 1) ; i < objectandsymboltypemapping.length ; i++ ){
  ...
  for(i = objectandsymboltypemapping.length; i>idxtoinsert; i--) {
...
function showchangeobjecttypedialog3(objecttypemapping, objectandsymboltypemapping, connectiontypemapping)
{
  ...
  userdialog.Text(20, 217, 190, 14, getString("TEXT21"));
...
function getconnectionsforobjecttypes(objecttype1, objecttype2)
{
  ...
  var i = 0; 
  var cxntypes = ofilter.CxnTypesFromObj(objecttype1.ntypenum, objecttype2.ntypenum);

  if (cxntypes.length>0) {
    connectiontypelist[0]          = new __usertype_tconnectiontype();
    connectiontypelist[0].ntypenum = 0;
    connectiontypelist[0].sname = getString("TEXT22");
    for ( i = 0; i < cxntypes.length; i++) {
      connectiontypelist[i+1]           = new __usertype_tconnectiontype();
      connectiontypelist[i+1].ntypenum  = cxntypes[i];
      connectiontypelist[i+1].sname     = ofilter.ActiveCxnTypeName(cxntypes[i]);
    }
  }

  __functionResult = connectiontypelist;
  return __functionResult;
}
...
function getobjecttypemappingconnections(objectandsymboltypemapping, objecttypemapping)
{
  ...
  for ( i = 0 ; i < objecttypemapping.value.length ; i++ ){
    ...
    for ( j = 0 ; j < partnerobjecttypes.length ; j++ ){
      targetobjecttype2 = getmappedobjecttype(partnerobjecttypes[j], objecttypemapping);

      cxntypes = ofilter.CxnTypesFromObj(targetobjecttype.ntypenum, targetobjecttype2.ntypenum);
      if (cxntypes.length > 0) {

        cxntypes = ofilter.CxnTypesFromObj(sourceobjecttype.ntypenum, partnerobjecttypes[j].ntypenum);
        if (cxntypes.length > 0) {

          for ( k = 0 ; k < cxntypes.length ; k++ ){
            ...
            objecttypemappingconnectiontypes[idx].sourceconnectiontype.sname    = ofilter.ActiveCxnTypeName(cxntypes[k]);
      ...
      // wenn nicht Beziehung innerhalb eines Objekttyps, umgekehrte Beziehungen ermitteln
        cxntypes = ofilter.CxnTypesFromObj(targetobjecttype2.ntypenum, targetobjecttype.ntypenum);
        if (cxntypes.length > 0) {
          cxntypes = ofilter.CxnTypesFromObj(partnerobjecttypes[j].ntypenum, sourceobjecttype.ntypenum);
          if (cxntypes.length > 0) {
            for ( k = 0 ; k < cxntypes.length ; k++ ){
            var idx = numobjecttypemappingconnectiontypes + k;
              objecttypemappingconnectiontypes[idx]                                 = new __usertype_tobjecttypemappingconnectiontypeentry();
              ...
              objecttypemappingconnectiontypes[idx].sourceconnectiontype.ntypenum   = cxntypes[k];
              objecttypemappingconnectiontypes[idx].sourceconnectiontype.sname      = ofilter.PassiveCxnTypeName(cxntypes[k]);
...
function getpartnerobjecttypesforconnection(sourceobjecttype, objecttypemapping)
{
  ...
  var i = 0;   var j = 0;   var idxtoinsert = -1;   var cxntypes = new Array(); 

  var allobjecttypes = initobjecttypes();

  // Cxn type nums from source object to connected objects
  for ( i = 0 ; i < allobjecttypes.length; i++ ){
    objecttype = allobjecttypes[i];
    cxntypes = ofilter.CxnTypesFromObj(sourceobjecttype.ntypenum, objecttype.ntypenum);
    if (cxntypes.length == 0) {
        // Vice versa - Cxn type nums from connected objects to source object (TANR 248686)
        cxntypes = ofilter.CxnTypesFromObj(objecttype.ntypenum, sourceobjecttype.ntypenum);        
    } 
    if (cxntypes.length > 0) {
      idxtoinsert = 0;
      for ( j = 0 ; j < partnerobjecttypes.length; j++ ){
      ...
      for(j = partnerobjecttypes.length; j>idxtoinsert; j--) {
...
function addconnectiontypemapping(sourceobjecttype1, sourceconnectiontype, sourceobjecttype2, targetobjecttype1, targetconnectiontype, targetobjecttype2, connectiontypemapping)
{
  ...
  for ( i = 0 ; i < connectiontypemapping.length ; i++ ){
  ...
  for(i = connectiontypemapping.length; i>idxtoinsert; i--) {
...
function addobjecttypemappingconnectiontype(sourceobjecttype1, sourceconnectiontype, sourceobjecttype2, targetobjecttype1, targetobjecttype2, objecttypemappingconnectiontypes)
{
  ...
  for ( i = 0 ; i < objecttypemappingconnectiontypes.length; i++ ){
  ...
  for(i = objecttypemappingconnectiontypes.length; i>idxtoinsert; i--) {
...
function getobjecttypemappingtexts(objecttypemapping)
{
  ...
  var result = new Array(); 
  for (var i = 0 ; i < objecttypemapping.length; i++ ){
    if (objecttypemapping[i].nattributefiltertypenum != 0) {
      result[i] = getObjectNameAndType(objecttypemapping[i].sourceobjecttype) + " -> " + 
                  getObjectNameAndType(objecttypemapping[i].targetobjecttype) + getString("TEXT28") + 
                  objecttypemapping[i].sattributefiltername + ")";
    } else {
      result[i] = getObjectNameAndType(objecttypemapping[i].sourceobjecttype) + " -> " + 
...
{
  var __functionResult;
  var result = new Array()  // __usertype_tobjecttype() 

  var ofilter = null; 
  ofilter = ArisData.getActiveDatabase().ActiveFilter();

  var typenums = new Array(); 
  typenums = ofilter.ObjTypes();

  for (var i = 0; i < typenums.length; i++) {
    result[i] = new __usertype_tobjecttype();
    result[i].ntypenum = typenums[i];
    result[i].sname = ofilter.ObjTypeName(typenums[i]);
  }
...
{
  var __functionResult;
  var result = new Array();     // __usertype_tattributetype() 

  var ofilter = ArisData.getActiveDatabase().ActiveFilter();

  // Hier noch weitere sinnvolle Werte angeben
  result[0] = new __usertype_tattributetype();
  result[0].ntypenum = Constants.AT_NAME;
  result[1] = new __usertype_tattributetype();
  result[1].ntypenum = Constants.AT_ID;
  result[2] = new __usertype_tattributetype();
  result[2].ntypenum = Constants.AT_DESC;
  result[3] = new __usertype_tattributetype();
  result[3].ntypenum = Constants.AT_CREATOR;

  for (var i = 0 ; i < result.length; i++) {
    result[i].sname = ofilter.AttrTypeName(result[i].ntypenum);
  }
...
function getsymboltypesforobject(objecttypenum, binsertnosymboltype)
{
  ...
  var modeltypenums = new Array();   var symboltypenums = new Array(); 
  var i = 0;   var j = 0;

  ofilter = ArisData.getActiveDatabase().ActiveFilter();

  modeltypenums = ofilter.ModelTypes(Constants.ARISVIEW_ORG | Constants.ARISVIEW_FUNC | Constants.ARISVIEW_DATA | Constants.ARISVIEW_CTRL);

  for ( i = 0; i < modeltypenums.length; i++) {
    symboltypenums = ofilter.Symbols(modeltypenums[i], objecttypenum);

    if (symboltypenums.length > 0) {
      for ( j = 0; j < symboltypenums.length; j++) {
        addtouniquesymboltypelist(symboltypelist, symboltypenums[j]);
      }
    }
  }

  if (binsertnosymboltype!=null && binsertnosymboltype) {
    for(i = symboltypelist.length; i>0; i--) {
      symboltypelist[i] = symboltypelist[i-1];
    }

    symboltypelist[0] = new __usertype_tsymboltype();
    symboltypelist[0].ntypenum = 0;
    symboltypelist[0].sname = getString("TEXT29");
  }

  __functionResult = symboltypelist;
  return __functionResult;
}


function addtouniquesymboltypelist(symboltypelist, ntypenum)
{
  var ofilter = null; 
  var i = 0;   var indextoinsert = 0;

  ofilter = ArisData.getActiveDatabase().ActiveFilter();

  indextoinsert = 0;

  for ( i = 0 ; i < symboltypelist.length; i++ ){
    if (symboltypelist[i].ntypenum == ntypenum) {
      indextoinsert = -1;
      break;
    } else if (symboltypelist[i].ntypenum > ntypenum) {
      indextoinsert = i;
      break;
    }

    if (i == (symboltypelist.length - 1)) {
      indextoinsert = symboltypelist.length;
    }
  }
  ...
  for ( i = symboltypelist.length ; i > indextoinsert; i-- ) {
    symboltypelist[i] = symboltypelist[i-1];
  }
  
  symboltypelist[indextoinsert] = new __usertype_tsymboltype();
  symboltypelist[indextoinsert].ntypenum = ntypenum;
//  symboltypelist[indextoinsert].sname = "Symbol #"+ntypenum;
  symboltypelist[indextoinsert].sname = ofilter.SymbolName(ntypenum) + " (#"+ntypenum+")";
}
...
function getobjecttypesforactivedatabase()
{
  ...
  var objtypenums = ofilter.ObjTypes();

  if (objtypenums.length == 0) {
    return __functionResult;
  }

  var i = 0; 
  var objecttypelist = new Array(); 
  var objecttype = new __usertype_tobjecttype(); 

  for ( i = 0 ; i < objtypenums.length ; i++ ){
    objecttype = new __usertype_tobjecttype();
    objecttype.ntypenum = objtypenums[i];
    objecttype.sname = ofilter.ObjTypeName(objtypenums[i]);
    addobjecttype(objecttype, objecttypelist);
  }

  __functionResult = objecttypelist;
...
function addobjecttype(objecttype, objecttypelist)
{
  var i = 0;   var idxtoinsert = 0; 
  idxtoinsert = -1;
  for ( i = 0 ; i < objecttypelist.length ; i++ ){
  ...
  for(i = objecttypelist.length; i>idxtoinsert; i--) {
...
// ------------------------------------------------------------
// Entfernt den Eintrag mit dem angegebenen Index aus dem Array
// 
function removeobjecttype(objecttypelist, index)
{
  ...
  if (objecttypelist.length > 1) {
    for (var i = index ; i < objecttypelist.length -1 ; i++ ){
      objecttypelist[i] = objecttypelist[i+1];
    }
    objecttypelist = truncarray(objecttypelist, 1);
  }
  else {
    objecttypelist = new Array();   //  __usertype_tobjecttype()
  } 
  return objecttypelist;
}
...
// Fuegt dem Array einen Eintrag hinzu
// 
function addsymboltype(symboltype, symboltypelist)
{
  var i = 0;   var idxtoinsert = 0; 
  idxtoinsert = -1;
  for ( i = 0 ; i < symboltypelist.length ; i++ ){
    if (StrCompIgnoreCase(symboltype.sname, symboltypelist[i].sname) < 0) {
      idxtoinsert = i;
      break;
    }
  }
  ...
  if (idxtoinsert == -1) {
    idxtoinsert = symboltypelist.length;
  }

  for(i = symboltypelist.length; i>idxtoinsert; i--) {
    symboltypelist[i] = symboltypelist[i-1];
  }

  symboltypelist[idxtoinsert] = symboltype;
}
...
// Entfernt den Eintrag mit dem angegebenen Index aus dem Array
// 
function removesymboltype(symboltypelist, index)
{
  if (symboltypelist.length == 0) {
    return symboltypelist;
  }

  if (symboltypelist.length > 1) {
    var i = 0; 
    for ( i = index ; i < symboltypelist.length -1 ; i++ ){
      symboltypelist[i] = symboltypelist[i+1];
    }

    symboltypelist = truncarray(symboltypelist,1);
  }
  else {
    symboltypelist = new Array();   // __usertype_tsymboltype() 
  }
  return symboltypelist;
}
...
function addobjecttypemapping(sourceobjecttype, targetobjecttype, objecttypemapping)
{
  ...
  var idxtoinsert = -1;
  for ( i = 0 ; i < objecttypemapping.length; i++ ){
    if (objecttypemapping[i].sourceobjecttype.ntypenum > sourceobjecttype.ntypenum) {
      idxtoinsert = i;
      break;
    }
  }
  ...
  if (idxtoinsert == -1) {
    idxtoinsert = objecttypemapping.length;
  }
  
  for(i = objecttypemapping.length; i> idxtoinsert; i--) {
    objecttypemapping[i] = objecttypemapping[i - 1];
  }

  objecttypemapping[idxtoinsert] = new __usertype_tobjecttypemappingentry();
  objecttypemapping[idxtoinsert].sourceobjecttype           = sourceobjecttype;
  objecttypemapping[idxtoinsert].targetobjecttype           = targetobjecttype;
  objecttypemapping[idxtoinsert].nattributefiltertypenum    = 0;
  objecttypemapping[idxtoinsert].sattributefiltername       = "";
}
...
// ------------------------------------------------------------
// Fuegt dem Array einen Eintrag mit Attributfilter hinzu und liefert das aktualisierte Array zurueck
// 
function addobjecttypemappingwithattributefilter(sourceobjecttype, targetobjecttype, nattributefiltertypenum, objecttypemapping)
{
  var ofilter = ArisData.getActiveDatabase().ActiveFilter();
  ...
  objecttypemapping[idxtoinsert].sourceobjecttype           = sourceobjecttype;
  objecttypemapping[idxtoinsert].targetobjecttype           = targetobjecttype;
  objecttypemapping[idxtoinsert].nattributefiltertypenum    = nattributefiltertypenum;
  objecttypemapping[idxtoinsert].sattributefiltername       = ofilter.AttrTypeName(nattributefiltertypenum);
}
...
// ------------------------------------------------------------
// Entfernt den Eintrag mit dem angegebenen Index aus dem Array und liefert das aktualisierte Array zurueck
// 
function removeobjecttypemapping(objecttypemapping, index)
{
  return removefromarray(objecttypemapping, index);
}

function debugout(dbgString) {
}
