...
/**
 * The RequestOperationHandler handles the requests for creation/deletion of systems/components
 *
 * @author Y10382
 */

function RequestOperationHandler() {
    ...
    var createdItems;
    var arisMetaModel;
    var requestModelFound;
    var modelsToLayout = new Packages.java.util.HashSet();
    var processedElements = new Packages.java.util.HashMap();
    ...
    this.handle = function(database, processor) {
        ...
        while ((operation = processor.next()) != null) {
            try {
                ...
                // distinguishing between two general operation properties - creating/deleting system/component & processing request status.
                // based on this, corresponding handling is selected
                operationProperty = "" + operation.getAttribute("property");
                // operation property is equal to 'requestStatus'
                if (operationProperty.equals("requestStatus")) {
                    var request = processRequestStatus(objectNode);
                    if (request)
                        processor.setStatus(ImportProvider.SUCCEEDED, ImportProvider.NO_ERROR, "Operation completed successfully.", -1, true, description);
                    else
                        processor.setStatus(ImportProvider.FAILED, ImportProvider.NOT_FOUND_ERROR, "Operation failed. Verifiying the request status was unsuccessful.", 0, true, description);

                    // operation property is about creating/deleting system/component
                } else {
                    ...
                    if (handled) {
                        ...
                        processor.setStatus(ImportProvider.FAILED, ImportProvider.NOT_FOUND_ERROR, "Operation failed. Handling the creation/deletion of system/component was unsuccessful.", 0, true, description);
                }
    ...
    // if property value is equal to "requestStatus" - it checks if the system/component with particular guid was created/deleted,
    // and if so, sends the information to ImportProvider
    function processRequestStatus(objectNode) {
        var guid = objectNode.getAttribute("value");
        var typeNum = objectNode.getAttribute("typeNum");
        var children = getRealChildren(objectNode);
        var check1 = false;
        var check2 = false;
        var systemType = null;

        for (var i = 0; i < children.length; i++) {
            // if function of attribute node is target, just check if the value is equal to 3
            var isTarget = children[i].getAttribute("function").indexOf("target") >= 0;
            if (isTarget) {
                if (children[i].getAttribute("value") == 3)
                    check1 = true;
            }

                // check if element with specified guid and typeNum was really successfully processed
                check2 = checkIfOperationCompleted(guid, typeNum);
        }

        if (delayedGUIDSet.contains(guid))
            throw new OperationException(ImportProvider.TEMPORARY_ERROR, "Context object is in delayed mode. Its changes must be delayed too.", -1, "aaa");

        // if both conditions are satisfied, return true (operation is therefore successful)
        return check1 && check2;
    }

    function checkIfOperationCompleted(guid, typeNum) {
        // typeNum = if element is e.g. Application system type
        var element = processedElements.get(guid);
        if (element == null)
            return false;
        else {
            if (element.getTypeNum() == 0)
                return true;

            if (element.getTypeNum() == typeNum)
                return true;
            else
                return false;
        }
    ...
    // finds out the type of element (system/component) and type of operation (create/delete)
    function processElementHandling(objectNode) {
        ...
        } else if (oFunction.equals("delete")) {
            processed = deleteElement(object, oTypeNum, oGuid);
        }
    ...
    function createElement(element, typeNum, guid) {
        ...
        if (theObject != null && theObject.IsValid()) {
            ...
            if ((model = processRequestModel(theObject, modelNode, M_CHECK)) != null) {
                ...
                var el = db.FindGUID(guid);
                var num = el.Attribute(3656, -1).MeasureUnitTypeNum();
                // add guid to processedElements (for purposes of verifying requestStatus)
                processedElements.put(guid, new RequestInfo(typeNum, num));
                return true;
            }
        }
    ...
    // handles deletion of specific element
    function deleteElement(element, typeNum, guid) {
        var elementChildren = getRealChildren(element);
        var attributes = getNamedChildren(elementChildren, "attribute");
        var num;

        // checks if object with specified guid really exists
        var theObject = db.FindGUID(guid);

        if (theObject != null && theObject.IsValid()) {
            contextItem = theObject;
            var el = db.FindGUID(guid);
            num = el.Attribute(3656, -1).MeasureUnitTypeNum();
            for (var i = 0; i < elementChildren.length; i++) {
                if (elementChildren[i].getTagName().equals("attribute"))
                    if (!processAttribute(theObject, elementChildren[i], 4))
                        throw new OperationException(ImportProvider.WRITE_ERROR, "Could not delete attribute");
            }

            var objGroup = theObject.Group();

            if (!objGroup.Delete(theObject))
                throw new OperationException(ImportProvider.WRITE_ERROR, "Could not delete the object with guid=" + guid);

            // delete also the group => not yet supported
            //objGroup.Parent().Delete(objGroup)


        } else {
            // object doesnt exist (it's not found)/is considered therefore as deleted, so we are done here
            processedElements.put(guid, new RequestInfo(0, 0));
            return true
        }

        // last check
        theObject = db.FindGUID(guid);
        if (theObject == null || !theObject.IsValid()) {
            // if it was really deleted put the object's guid into 'processedElements' for purpose of verifying in requestStatus operation
            processedElements.put(guid, new RequestInfo(typeNum, num));
            return true;
        }
        else
    ...
    function RequestInfo(typeNum, type) {
    ...
    function processRequestModel(parentItem, node, mode) {
        return bestMatchSupport.processModelApplyingBestMatchApproach(parentItem, node, mode, requestModelFound);
    }
