...
var g_nnumcharsforwidth2 = 0;       // Maximale Laenge Teilstring ohne Leerzeichen im Objektnamen, ab denen die WKD-Symbolgrösse 2 verwendet wird 
var g_nnumcharsforwidth3 = 0;       // Maximale Laenge Teilstring ohne Leerzeichen im Objektnamen, ab denen die WKD-Symbolgrösse 3 verwendet wird 
...
function main() {
    ...
    if (omodellist.length > 0) {
        ...
        for (var i = 0 ; i < omodellist.length ; i++ ) {
            ...
            if (isvalidmodeltype(omodel)) {
                ...
                outTable(omodel);        // Anubis 425370 - Out table
    ...
    g_oOutfile.WriteReport(false);    
    
    if (bhandled) {
        copyStaticFiles();
        copySymbolFiles();      // BLUE-8302 
    }   
}
...
function handlemodel(omodel) {
    ...
    oobjocclist = filterObjOccsWithMaintainedAttributes(oobjocclist);   // Anubis 425370: Objects with missing attributes are filtered out
...
//  g_oOutfile.OutputTxt(TAB + TAB + "<colgroup><col width=\'60\'><col width=\'*\'><col width=\'60\'></colgroup>" + CRLF);
    ...
    g_oOutfile.OutputTxt(CRLF);
/*    
    // Anubis 425370: Commented out (to output table)
    g_oOutfile.OutputTxt(TAB + "</body>" + CRLF);
...
function getIniEntry_string(p_sEntry) {
    var sFile = "processEvaluation.xml"; 
    var sSection = "PROCESSEVALUATION"; 
...
function copySymbolFiles() {
    if (isOldPalette()) {
        // Copy "wkd_<x>.gif"
        Context.addOutputFileName("wkd_1.gif", Constants.LOCATION_SCRIPT);
        Context.addOutputFileName("wkd_2.gif", Constants.LOCATION_SCRIPT);
        Context.addOutputFileName("wkd_3.gif", Constants.LOCATION_SCRIPT);
    } else {
        // Rename and copy "wkd_<x>_new.gif" -> "wkd_<x>.gif" (BLUE-8302)
        copyAndRename("wkd_1_new.gif", "wkd_1.gif");
        copyAndRename("wkd_2_new.gif", "wkd_2.gif");
        copyAndRename("wkd_3_new.gif", "wkd_3.gif");
    }  

    function copyAndRename(srcName, trgName) {
        var res = Context.getFile(srcName, Constants.LOCATION_SCRIPT);
        Context.setFile(trgName, Constants.LOCATION_OUTPUT, res);
    }              
...
function filterObjOccsWithMaintainedAttributes(oObjOccList) {
    var oFilteredObjOccs = new Array();
    for (var i = 0; i < oObjOccList.length; i++) {
        var oObjOcc = oObjOccList[i];
        if (areAttributesMaintained(oObjOcc)) oFilteredObjOccs.push(oObjOcc);
    }
    return oFilteredObjOccs;
}

function areAttributesMaintained(oObjOcc) {
    var oObjDef = oObjOcc.ObjDef();
    if (!oObjDef.Attribute(g_nxattribtype, g_nloc).IsMaintained()) return false;
    if (!oObjDef.Attribute(g_nyattribtype, g_nloc).IsMaintained()) return false;
    return true;
}

function outTable(oModel) {
    var nColor_Head = Constants.C_GREY_80_PERCENT;
    
    g_oOutfile.OutputTxt("<div style=\'position:absolute; top:700px\'>" + CRLF);
    
    g_oOutfile.BeginTable(100, Constants.C_BLACK, Constants.C_TRANSPARENT, Constants.FMT_LEFT, 0);          
    g_oOutfile.TableRow();
    g_oOutfile.TableCell(getString("TABLE_OBJECT_NAME"), 25, getString("TABLE_FONT"), 10, Constants.C_BLACK, nColor_Head, 0, Constants.FMT_BOLD | Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
    g_oOutfile.TableCell(getString("TABLE_SYMBOL_TYPE"), 25, getString("TABLE_FONT"), 10, Constants.C_BLACK, nColor_Head, 0, Constants.FMT_BOLD | Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
    g_oOutfile.TableCell(getAttrTypeName(g_nxattribtype), 25, getString("TABLE_FONT"), 10, Constants.C_BLACK, nColor_Head, 0, Constants.FMT_BOLD | Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
    g_oOutfile.TableCell(getAttrTypeName(g_nyattribtype), 25, getString("TABLE_FONT"), 10, Constants.C_BLACK, nColor_Head, 0, Constants.FMT_BOLD | Constants.FMT_LEFT | Constants.FMT_VTOP, 0);

    var bColored = false;

    var oObjOccList = oModel.ObjOccListFilter(g_nobjtype);
    oObjOccList = ArisData.sort(oObjOccList, Constants.AT_NAME, g_nloc);
    for (var i = 0; i < oObjOccList.length; i++) {
        var oObjOcc = oObjOccList[i];
        var oObjDef = oObjOcc.ObjDef();
        
        var fontColor = getFontColor(oObjOcc);
        g_oOutfile.TableRow();
        g_oOutfile.TableCell(oObjDef.Name(g_nloc), 25, getString("TABLE_FONT"), 10, fontColor, getTableCellColor_AttrBk(bColored), 0, Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
        g_oOutfile.TableCell(oObjOcc.SymbolName(), 25, getString("TABLE_FONT"), 10, fontColor, getTableCellColor_AttrBk(bColored), 0, Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
        g_oOutfile.TableCell(getAttrValue(oObjDef, g_nxattribtype), 25, getString("TABLE_FONT"), 10, fontColor, getTableCellColor_AttrBk(bColored), 0, Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
        g_oOutfile.TableCell(getAttrValue(oObjDef, g_nyattribtype), 25, getString("TABLE_FONT"), 10, fontColor, getTableCellColor_AttrBk(bColored), 0, Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
        bColored = !bColored;
    }
    g_oOutfile.EndTable("", 100, getString("TABLE_FONT"), 12, Constants.C_BLACK, Constants.C_TRANSPARENT, 0, Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
    
    g_oOutfile.OutputTxt("</div>" + CRLF);

    function getFontColor(oObjOcc) {
        if (!areAttributesMaintained(oObjOcc)) return Constants.C_RED;
        return Constants.C_BLACK;   // default
    }
    
    function getAttrTypeName(nAttrTypeNum) {
        return "" + ArisData.ActiveFilter().AttrTypeName(nAttrTypeNum);
    }
    
    function getAttrValue(oItem, nAttrTypeNum) {
        var oAttr = oItem.Attribute(nAttrTypeNum, g_nloc);
        if (oAttr.IsMaintained()) return oAttr.GetValue(true);
        return "";
    }
}
