...
var g_headers = new Array("Model Old Name", "Model New Name", "Model Type", "Location", "Log");
...
for (var i = 0; i < fadModels.length; i++) {
    ...
    var oldName = fadModels[i].Name(nLocale)
    
    if(funcObjs.length != 0){
    if (oldName != funcObjs[0].Name(nLocale)) {
        var att = fadModels[i].Attribute(Constants.AT_NAME, nLocale)
        var attSet = att.setValue(funcObjs[0].Name(nLocale))

        if (attSet == true) {
            g_table.push([oldName, fadModels[i].Name(nLocale), fadModels[i].Type(), fadModels[i].Group().Path(nLocale), "Name successfully changed"]);
        } else {
            g_table.push([oldName, fadModels[i].Name(nLocale), fadModels[i].Type(), fadModels[i].Group().Path(nLocale), "Error, name is not changed"]);
        }
    }
    }
}
