...
var gPageSize="A0"; //Context.getProperty("pageSize");
var gExludeAttrNr=ArisData.ActiveFilter().UserDefinedAttributeTypeNum("6fcf1e11-d899-11e7-27f1-00155dff4f3e");
var gOuterTableWidth=0;
var gOuterTableHeight=0;
var gGraphicsHeight=0;
var gFirstOuterColumnWidth=30;
var gFirstOuterRowHeight=40;
var gUnusedOutputObject =null;
...
function main(){
    ...
    for(var n  in selModels){ 
        ...
        if(funcObjDefs.length >0){
            ...
            if(gPageSize=="A0"){
                nrOfProcesses=nrOfProcesses*2;
            }
            var minZoomFactor=-1;
            var nrOfGraphicPerPage=nrOfProcesses;
            if(funcObjDefs.length<nrOfProcesses) nrOfGraphicPerPage = funcObjDefs.length;
            
            for each(var funcObjDef in funcObjDefs){
               var assignedModels=funcObjDef.AssignedModels([gFuncAssignedModelTypeMap.get(funcObjDef.GUID())]);
               for each(var  assignedModel in assignedModels){
                    
                    var zoomFactor=determineZoomFactor(assignedModel,nrOfGraphicPerPage);
                    if(minZoomFactor==-1 || zoomFactor<minZoomFactor){
            ...
            if(funcObjDefs.length <= nrOfProcesses){
                createPage(selModel,funcObjDefs.length, funcObjDefs, 0,minZoomFactor);
            }else{                
                ...
                do{                    
                    ...
                    createPage(selModel,nrOfProcesses, funcObjDefs, i,minZoomFactor);
                    i+=nrOfProcesses;
...
function createDocument(){
    gUnusedOutputObject = Context.createOutputObject();            
    gUnusedOutputObject.OutputLn("unused", "Arial", 10, Constants.C_BLACK, Constants.C_TRANSPARENT, Constants.FMT_LEFT, 0);
    gUnusedOutputObject.WriteReport();
    ...
    gOuterTableWidth=gPdfSettings.pageSize.getWidth()-gPdfSettings.leftMargin-gPdfSettings.rightMargin;
    gOuterTableHeight=gPdfSettings.pageSize.getHeight()-gPdfSettings.topMargin-gPdfSettings.bottomMargin;
    gGraphicsHeight=0.5*(gOuterTableHeight-gFirstOuterRowHeight);
...
function closeDocument(){
    gDocument.close();          
    var lFileInternalOutputPath = gUnusedOutputObject.getCurrentPath();//returns a java.io.File object
...
function createPage(pSelModel,piNrOfProcesses,pFuncObjDefs, pStartIndex,pMinZoomFactor){
    var lContentPadding=5;       
            
    var intArray = java.lang.reflect.Array.newInstance(java.lang.Integer.TYPE, piNrOfProcesses+1);
    
    var actRespHeight=0.5*(gOuterTableHeight-gFirstOuterRowHeight)*3/8;
    var actDescHeight=0.5*(gOuterTableHeight-gFirstOuterRowHeight)*5/8;
    
    var lProcessWidth=(gOuterTableWidth-gFirstOuterColumnWidth)/piNrOfProcesses;
    intArray[0]=1000*gFirstOuterColumnWidth/gOuterTableWidth;
    for(var i=1;i<=piNrOfProcesses;i++){
        intArray[i]=1000*((gOuterTableWidth-gFirstOuterColumnWidth)/piNrOfProcesses)/gOuterTableWidth;
    }
    
    var table = new com.lowagie.text.pdf.PdfPTable(piNrOfProcesses+1);
    table.setWidths(intArray);
    table.setTotalWidth(gOuterTableWidth);
    table.setWidthPercentage(100);
    
    var tableHeaderFont = getTextFont(true,24);
    ...
    tcell.setFixedHeight(gFirstOuterRowHeight);    
    ...
    //print graphics
    var tcell = new com.lowagie.text.pdf.PdfPCell(new com.lowagie.text.Phrase("Process Graphics", rowHeaderFont));
    tcell.setFixedHeight(gGraphicsHeight);
    ...
    for(var i=0;i<piNrOfProcesses;i++){
        var modelGraphicTable=getModelGraphicTable(pFuncObjDefs[pStartIndex+i],pMinZoomFactor)
        var tcell=new com.lowagie.text.pdf.PdfPCell(modelGraphicTable);
        tcell.setHorizontalAlignment(com.lowagie.text.Element.ALIGN_CENTER);
        tcell.setVerticalAlignment(com.lowagie.text.Element.ALIGN_MIDDLE);
    ...
    for(var i=0;i<piNrOfProcesses;i++){        
        //var activityTables=createActivityTables(pFuncObjDefs[pStartIndex+i],(gOuterTableHeight-gFirstOuterRowHeight)/3,lProcessWidth);
        var tcell=new com.lowagie.text.pdf.PdfPCell(processActivityTables[i][0]);
        //tcell.setFixedHeight((gOuterTableHeight-gFirstOuterRowHeight)/3);
        tcell.setPadding(lContentPadding);
        tcell.setBorderColor(gBGColor.processHeaderCell);
        table.addCell(tcell);
    }
...
function printFooterAndHeader(pSelModel){
    ...
    var tableHeaderFont = getTextFont(true,36);
    ...
    var canvas = gWriter.getDirectContent();
    table.writeSelectedRows(0, 6, gPdfSettings.leftMargin, gPdfSettings.pageSize.getHeight() - gPdfSettings.topMargin+50, canvas);
...
function getModelGraphicTable(pObjDef,piMinZoomFactor){
    var modelGraphicTable = new com.lowagie.text.pdf.PdfPTable(1);
    var assignedModels=pObjDef.AssignedModels(gFuncAssignedModelTypeMap.get(pObjDef.GUID()));
    if(assignedModels.length>0){         
        var lsPictureFileName = "modelgraph.png";        
        var lModel=assignedModels[0];
        lModel.BuildGraph(true);
        
        var lModelPicture = lModel.Graphic(false, false, Context.getSelectedLanguage());
        lModelPicture.setZoom(piMinZoomFactor*100);
               
        var lbResult = lModelPicture.Save(gUnusedOutputObject, lsPictureFileName);
        //var lModelImage = javax.imageio.ImageIO.read(new java.io.ByteArrayInputStream(Context.getFile(lsPictureFileName, Constants.LOCATION_OUTPUT))); 
        var lModelImage=com.lowagie.text.Image.getInstance(Context.getFile(lsPictureFileName, Constants.LOCATION_OUTPUT))
        var tcell=new com.lowagie.text.pdf.PdfPCell(lModelImage, false);
        tcell.setHorizontalAlignment(com.lowagie.text.Element.ALIGN_CENTER);
        tcell.setVerticalAlignment(com.lowagie.text.Element.ALIGN_MIDDLE);
        tcell.setBorderWidth(0);
        modelGraphicTable.addCell(tcell);
    }
    return modelGraphicTable;
}
...
function determineZoomFactor(pModel,piNumberOfModels){
    ...
    var lWidthPixelNZ = lModelImage.getWidth();
    var lHeightPixelNZ = lModelImage.getHeight();
   
    //var lWidthLoMetric = lModelPicture.getWidth(Constants.SIZE_LOMETRIC);
    //var lHeightLoMetric = lModelPicture.getHeight(Constants.SIZE_LOMETRIC);
            
    var lZoomFactorHeight=(gGraphicsHeight-2*lCellMargin)/lHeightPixelNZ;
    var lZoomFactorWidth=(gOuterTableWidth-2*piNumberOfModels*lCellMargin)/(lWidthPixelNZ*piNumberOfModels);
    
    if(lZoomFactorHeight<lZoomFactorWidth){
        return lZoomFactorHeight;        
    }else{
        return lZoomFactorWidth;       
    }
}
