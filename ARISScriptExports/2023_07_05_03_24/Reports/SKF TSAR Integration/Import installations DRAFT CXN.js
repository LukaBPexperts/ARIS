...
g_mapTable.push(["Installation ID", "AT_ID"]);    //PPM ID
g_mapTable.push(["Installation Name", Constants.AT_NAME]);                           //Name
...
----------------------------------------------------------------------------- */
var g_applicationID = "Installation ID";                                                          //system ID column header in import file, used for matching system between ARIS and PPM
var g_activeColumn = "Active"
var g_parentCxnIDColumn = "System ID"
...
var g_applicationObjType = Constants.OT_APPL_SYS_TYPE;          //system object type
var g_appMainObjType = Constants.OT_APPL_SYS_CLS;               //application object type

var g_applicationObjSymbolType = "3d8757f1-e5cb-11e5-6fc6-005056a95aef"                        //IT system symbol type
//Application Instance	Application system type	458785	3d8757f1-e5cb-11e5-6fc6-005056a95aef	Derived	

var g_applicationModelName = "System/Installation Map";        //Name of model
var g_applicationModelType = "2c1d6381-d494-11e5-6fc6-005056a95aef" //Constants.MT_APPL_SYS_TYPE_DGM;              //system landscape model

//var g_applicationModelName2 = "Applications map"; 





var g_applicationOrgCxnType = Constants.CT_BELONGS_TO_CLS; //Installation-Application relationship type  CUSTOM
var g_parentCxnObjType = Constants.OT_APPL_SYS_CLS; //Application object type (in this case System object type)
...
function main(){
    ...
    if (workbookReader != null) {
        ...
        var libraryModel = findDeleteAndCreateLibraryModel(s_libraryModelGroup, s_libraryModelType, g_applicationModelName, g_oOutfile); //custom
        ...
        updateLibraryModel(libraryModel, s_importGroup, g_applicationObjType, g_appMainObjType);
        
        //var applicationModel = findOrCreateLibraryModel(s_libraryModelGroup, s_libraryModelType, g_applicationModelName2, g_oOutfile);
        //updateCxnLibraryModel(applicationModel) //CUSTOM commented for now, creating cxns in different way
...
function readImportExcel(sheet){
    ...
    var parentCxnIDColumn = 0; //CUSTOM
    ...
    //read header row to get attributes and IDColumn and Active column      
    var row = 0;
    ...
    for (var column = 0; column < nrOfColumns; column++){
        ...
        else if (columnHeader == g_activeColumn){ //Mark Active column
        activeColumn = column
        }
        else if (columnHeader == g_parentCxnIDColumn){ //Mark Parent Cxn column
        parentCxnIDColumn = column
        }
    ...
    for (row = 1; row < sheet.getLastFilledRowNum()+1; row++){
        ...
        //var oCellType = sheet.getCell(row, activeColumn).getCellType().getType();
        
        var objActiveCell = sheet.getCell(row, activeColumn)
        if(objActiveCell != null){
            var objActive = sheet.getCell(row, activeColumn).getCellValue();
        }
        else{
            var objActive = ""
        }
        ...
        //*****************************************************
        //create cxn to Parent Object CUSTOM
        var parentCxnID = ""
        var selCell = sheet.getCell(row, parentCxnIDColumn);
        parentCxnID = getOriginalCellValue(selCell);
        
        var searchObjTypeNumber = g_parentCxnObjType
        var searchItem = g_currentDB.createSearchItem(IDArisTypeNr, g_nEng, parentCxnID, Constants.SEARCH_CMP_EQUAL, false, false);
        var targetObjects = s_importGroup.ObjDefList(true, searchObjTypeNumber, searchItem);
        
        if (targetObjects.length > 0){
            //create cxn
           var cxnTemp = oObject.CreateCxnDef(targetObjects[0], Constants.CT_BELONGS_TO_CLS) //add cxn type script header
...
//CUSTOM
function findDeleteAndCreateLibraryModel(p_modelGroup, p_modelType, p_modelName, p_oOutfile){
    var libraryModel;
    var searchItem = g_currentDB.createSearchItem(Constants.AT_NAME, g_nEng, p_modelName, Constants.SEARCH_CMP_EQUAL, false, false);
    var candidateModels = p_modelGroup.ModelList(false, [p_modelType], searchItem);
    if (candidateModels.length > 0){
        ...
        if (oLockOwner != ""){
           ...
           libraryModel = null;
            ...
            var deleteModel = p_modelGroup.Delete(libraryModel);
            libraryModel = p_modelGroup.CreateModel(p_modelType, p_modelName, g_nEng);
        }
    }else{
        libraryModel = p_modelGroup.CreateModel(p_modelType, p_modelName, g_nEng);
...
function updateLibraryModel(p_libraryModel, p_objGroup, p_objType, p_targetObjType){
    var xInit = 50;
    var xStep = 500;
    //var xMax = 4000;
    var xMax = 25000;
    ...
    var appObjects = p_objGroup.ObjDefListFilter(p_targetObjType);
    //allObjects = ArisData.sort(allObjects, Constants.AT_NAME, g_nEng);
    appObjects = ArisData.sort(appObjects, Constants.AT_NAME, g_nEng);
    
    for (var i=0; i<appObjects.length; i++){
        var oObjectDef = appObjects[i];
        if(oObjectDef.OccListInModel(p_libraryModel).length == 0){
            var defSymbolType = oObjectDef.getDefaultSymbolNum(); //custom
            //var defSymbolType = g_applicationObjSymbolType
            xPos = xPos + xStep;
            if(xPos > xMax){               
                xPos = xInit; //this is ok
                //check again last occurence position y
                    //Find last occurrence position in libraryModel
                var allOccs = p_libraryModel.ObjOccList();
                for (var i=0; i<allOccs.length; i++){
                    var testOcc = allOccs[i];
                ...
                yPos = yPos + yStep; //yPos is last occurence yPos plus yStep
            }
            ...
            var inCxnDefs = oObjectDef.CxnListFilter(Constants.EDGES_INOUT, g_applicationOrgCxnType);
            //set initial x y coordinates for system (app x y plus something)
            var xPosSys = xPos
            var yPosSys = yPos + yStep
            //for each system 
            for (var j=0; j<inCxnDefs.length; j++){
                ...
                var cxnOccsInModel = currCxnDef.OccList([p_libraryModel]);               
                //create objOcc
                if (cxnOccsInModel.length == 0){ //create new connection occ
                    var oSourceObjDef = currCxnDef.SourceObjDef();
                    var oSourceOccs = oSourceObjDef.OccListInModel(p_libraryModel);
                    var oSourceOcc;
                    if (oSourceOccs.length == 0){
                        var defSourceSymbolType = oSourceObjDef.getDefaultSymbolNum();
                        oSourceOcc = p_libraryModel.createObjOcc(defSourceSymbolType, oSourceObjDef, xPosSys, yPosSys, true, false); 
                        oSourceOcc.setZOrder(2)
                    } else {
                    ...
                    if (!oSourceOcc.IsValid()){
                        ...
                        yPosSys = yPosSys + yStep //move yPos step down
                    }
                                    
                    var g_ntranspoint = new Array();
                    g_ntranspoint[0] = new PublicInterface.Point(oSourceOcc.X() + oSourceOcc.Width() / 2, oSourceOcc.Y() - oSourceOcc.Height());
                    g_ntranspoint[1] = new PublicInterface.Point(newOcc.X() + newOcc.Width() / 2, newOcc.Y() + newOcc.Height());
                    p_libraryModel.CreateCxnOcc(oSourceOcc, newOcc, currCxnDef, g_ntranspoint, false);
...
function updateCxnLibraryModel(p_cxnLibraryModel){
    
    //var fromNoticeTypeNr = toTypeNr(g_fromNoticeAttribute);
    
    var p_targetObjects = p_cxnLibraryModel.ObjDefListByTypes([Constants.OT_APPL_SYS_CLS])
...
/*     try {
    ...
    for (var i=0; i<p_targetObjects.length; i++){
        ...
        var inCxnDefs = oTargetObjDef.CxnListFilter(Constants.EDGES_INOUT, g_applicationOrgCxnType);
        for (var j=0; j<inCxnDefs.length; j++){
            var currCxnDef = inCxnDefs[j];
            //var isImportedCxn = currCxnDef.Attribute(fromNoticeTypeNr, g_nEng).GetValue(false);
 ...
 //var layout = p_cxnLibraryModel.doLayout()
