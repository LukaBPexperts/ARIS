 ...
 * xmlRoot              org.jdom.Element
 */
var outfile = Context.createOutputObject();
var fileBytes = Context.getProperty("FILE_BYTES");
var nsARIS = org.jdom.Namespace.getNamespace("aris", "http://www.softwareag.com/BPMN/20151112/EXTENSION");
var nsBPMN = org.jdom.Namespace.getNamespace("bpmn", "http://www.omg.org/spec/BPMN/20100524/MODEL");
var nsBPMNDI = org.jdom.Namespace.getNamespace("bpmndi", "http://www.omg.org/spec/BPMN/20100524/DI");
var nsDC = org.jdom.Namespace.getNamespace("dc", "http://www.omg.org/spec/DD/20100524/DC");
var nsDI = org.jdom.Namespace.getNamespace("di", "http://www.omg.org/spec/DD/20100524/DI");
...
function getXmlContent() {
    ...
    if (typeof(xmlOutputter.getDocumentContent) === "function") {
        ...
        var jdomOutputter = new org.jdom.output.XMLOutputter(org.jdom.output.Format.getPrettyFormat().setEncoding("UTF-8"));
...
function computeCollaborations(xmlElement) {
    ...
    for (var iterator = collaborations.iterator(); iterator.hasNext();) {
        ...
        var collaborationChildren = collaboration.getDescendants(new org.jdom.filter.ElementFilter());
...
function mapDiagramToNewModel(diagram) {
    ...
    var children = diagram.getDescendants(new org.jdom.filter.ElementFilter());
...
function getNewXPathInstance(pattern) {
    var instance = org.jdom.xpath.XPath.newInstance(pattern);
...
function setArisAttribute(parent, typeNum, value) {
    ...
    if (attributeElement == null) {
        ...
        attributeElement = new org.jdom.Element("attribute", attributesElem.getNamespace());
...
function getOrCreateChild(parent, childName, childNamespace) {
    ...
    if (child === null) {
        ...
        child = new org.jdom.Element(childName, namespace);
...
function BPMNNormalizer(xmlElement) {
    ...
    this.forceIdAttribute = function () {
        ...
        while (element instanceof org.jdom.Element) {
            element = element.getParent();
        }

        var children = element.getDescendants(new org.jdom.filter.ElementFilter());
        while (children.hasNext()) {
            var child = children.next();
    ...
    this.addDataStoreDummies = function() {
        ...
        for (var i = 0; i < dataStoreReferences.size(); i++) {
            ...
            if (dataStoreRef == null) {
                ...
                var dataStore = new org.jdom.Element("dataStore", nsBPMN);
...
function SAPNormalizer(xmlElement) {
    ...
    this.createXPointElement = function (type) {
        var xpoint = new org.jdom.Element(this.xPointElementName);
