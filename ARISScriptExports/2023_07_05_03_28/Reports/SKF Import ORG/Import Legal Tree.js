...
var rowCount =  sheets[1].getRows().length
...
for (var i = 1; i < rowCount; i++) {
    ...
    try{
    var idSource = sheets[1].getCell(i, sourceIDColumn).getCellValue().trim() //get ID from excell
    var idTarget = sheets[1].getCell(i, targetIDColumn).getCellValue().trim() //get ID from excell
    }catch(e){}
    ...
    if(idSource != "" && idTarget != ""){
        ...
        if (!checkObjDef(sourceObjDef)) {
            var nameSource = sheets[1].getCell(i, 5).getCellValue() //get ID from excell
        ...
        if (!checkObjDef(targetObjDef)) {
            var nameTarget = sheets[1].getCell(i, 3).getCellValue() //get ID from excell
        ...
        if(importAtt == true){
            ...
            var locationAttTarget = ""
            var descriptionAttSource = ""
            var descriptionAttTarget = ""
            
            try{
            var nameSource = sheets[1].getCell(i, 5).getCellValue() //get Name from excell
            var nameTarget = sheets[1].getCell(i, 3).getCellValue() //get Name from excell
            
            var countryAttSource = sheets[1].getCell(i, 7).getCellValue() 
            //var countryAttTarget = sheets[1].getCell(i, 5).getCellValue() 
            
            //var descriptionAttTarget = sheets[1].getCell(i, 12).getCellValue() 
            //var descriptionAttTarget = sheets[1].getCell(i, 5).getCellValue() 
            }catch(e){}
            
            
            
            var nameSource = nameSource + " (" + idSource + ")"  
            ...
            //if(newTarget){
                // var nameTarget = nameTarget + " (" + idTarget + ")"  
            ...
            if(sourceObjDef.Attribute(countryAttNum, nLocale).getValue() != countryAttSource){
                var oldValue = sourceObjDef.Attribute(countryAttNum, nLocale).getValue()
                var updateCountryAttSource = sourceObjDef.Attribute(countryAttNum, nLocale).setValue(countryAttSource) 
                printRowSheet2(sourceObjDef.Name(nLocale), idSource, "Country Attribute Updated", oldValue, gExcelObject)
            }
            
            //var updateDescTarget = sourceObjDef.Attribute(descriptionAttNum, nLocale).setValue(descriptionAttTarget)
        }
    }
}
