...
function main() {
    ...
    try {                
        ...
        if(hasModelProceeInterfaces(oProcess)) {
            throw new Exception(commonUtils.attsall.formatString(getString("TEXT_PROCESS_INTERFACES"), [wizardResult.sProcessName]));
        }
//        outputStatistic();

        if (SHOW_DIALOGS) {
             Dialogs.MsgBox(commonUtils.attsall.formatString(getString("TEXT_MODEL_CREATED"), [oSelectedGroup.Name(gnLocale)]));
        }   
...
* oModel: epc
* return: true => contains process interface
*/
function hasModelProceeInterfaces(oModel) {
