...
function checkAttributeConditionDefined(pCondition, pSPC) {
    if ( pCondition.toUpperCase().indexOf("RANGE") == -1 ) {
        ...
        var typeNum = getTypeNum(pCondition);
...
function validateAdditionalAttributeCondition(pObject, pBaseField, pAdditionalCondition, pAttributeValue, aFieldNotFilled, pSPC) {
    ...
    // pAdditionalCondition consists of a single attribute name
       ...
       var id = getTypeNum(pAdditionalCondition);
 ...
 */
function getConnectionCountValidationOutput(pOccurenceMin, pOccurenceMax, pObjDef, pConnectedOjbDefs, pCheckedObjectString, pSPC){
    
    var szMsg = "";
    if (pSPC == null) {pSPC = "";}
    
    var connections = 0;
	if (pConnectedOjbDefs != null) {
        for (var i=0; i<pConnectedOjbDefs.length; i++) {
			if ( !isboolattributetrue(pConnectedOjbDefs[i], Constants.AT_DEACT, gLanguage) 
				 && !isLinkDeactivated(pObjDef, pConnectedOjbDefs[i])) {
				connections++;
			}
		}
    }
    ...
    if (connections < pOccurenceMin || connections > pOccurenceMax) {
        ...
        if (pConnectedOjbDefs != null && pConnectedOjbDefs.length > 0){
            for (var ou = 0; ou < pConnectedOjbDefs.length; ou++){
                szMsg += "\r\n" + pSPC + SPC1 + pConnectedOjbDefs[ou].Name(gLanguage);
            }  
        }
   }   
   return szMsg;
