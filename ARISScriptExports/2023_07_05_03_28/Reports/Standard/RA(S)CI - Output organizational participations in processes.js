...
function getData(oFuncObjects, p_oOrgObjects, oProcessModel) {
    ...
    for (var i = 0; i < oFuncObjects.length; i++) {
        ...
        var oOrgObjects;   
        if(p_oOrgObjects.length==0) {
            oOrgObjects = getConnectedInventoryObjects(oFuncObject);
        }
        else {
            oOrgObjects = p_oOrgObjects;
        }
...
function getConnectedInventoryObjects(oFuncObjDef) {
    var cxns = oFuncObjDef.CxnList( Constants.EDGES_INOUT )
    var result = new Array()
    for (var i in cxns) {
        var oCxn = cxns[i]
        if( oCxn.isInventoryCxn() ) {
            if(oCxn.TargetObjDef().equals(oFuncObjDef))
                result.push(oCxn.SourceObjDef());    
            else
                result.push(oCxn.TargetObjDef());    
        }
    }
    return result;        
}
...
function getRASCICxns(p_oFuncObject, p_oOrgObject, p_oModel) {
    ...
    oRelevantCxns = oRelevantCxns.concat(getRelevantCxnDefs(p_oFuncObject)); //BLUE-20149, consider cxns maintained in properties panel
...
function showOptionsDialog(bSelectHierarchyLevel) {
    ...
    while (binput == false && ! (nuserdlg == 1)) {
        ...
        if (bSelectHierarchyLevel) {
            ...
            userdialog.TextBox(185, 85, 60, 21, "txtLinkLevels");
        }
        userdialog.OKButton();        
