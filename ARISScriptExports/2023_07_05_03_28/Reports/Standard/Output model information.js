...
function main()
{
  ...
  var ocurrentcxnocc = null;   // Current connection occurrence.
  ...
  if (omodels.length > 0) {
    ...
    var aTitleDetails = [];
    if (omodels.length == 1) {
        aTitleDetails.push([getString("TEXT_2"), omodels[0].Name(nloc)]);
    }
    ...
    for (var i = 0 ; i < omodels.length; i++) {
        ...
        if (isSpreadsheet()) ooutfile.value.BeginTable(100, COL_TBL_BORDER, Constants.C_TRANSPARENT, Constants.FMT_LEFT, 0);
        // Model heading
        var sModelName = ocurrentmodel.Name(nloc);
        outHeading(ooutfile.value, sModelName, 1);
        // Properties
        if (isSpreadsheet()) outEmptyRow(ooutfile.value);
        ...
        if (!isSpreadsheet()) ooutfile.value.EndTable("", 100, getString("FONT"), FONT_SIZE, Constants.C_BLACK, Constants.C_TRANSPARENT, 0, Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
        ...
        if (bobjects) {
            ...
            for (var j=0; j<aoObjOccs.length; j++) {
                ...
                if (!isSpreadsheet()) ooutfile.value.EndTable("", 100, getString("FONT"), FONT_SIZE, Constants.C_BLACK, Constants.C_TRANSPARENT, 0, Constants.FMT_LEFT, 0);
                ...
                if (bobjcxn == true) {
                    ...
                    if (ocxnoccs.length > 0) {
                        ocxnoccs = ArisData.sort(ocxnoccs, Constants.AT_TYPE_6, Constants.SORT_NONE, Constants.SORT_NONE, nloc);
                        if (!isSpreadsheet()) ooutfile.value.BeginTable(100, COL_TBL_BORDER, Constants.C_TRANSPARENT, Constants.FMT_LEFT, 0);   
                        ...
                        for (var k=0; k<ocxnoccs.length; k++) {
        ...
        if (isSpreadsheet()) ooutfile.value.EndTable(sModelName, 100, getString("FONT"), FONT_SIZE, Constants.C_BLACK, Constants.C_TRANSPARENT, 0, Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
...
function outEmptyResult() {
    ...
    if (isSpreadsheet()) {
        ooutfile.BeginTable(100, COL_TBL_BORDER, Constants.C_TRANSPARENT, Constants.FMT_LEFT, 0);
        ooutfile.TableRow();
        ooutfile.TableCell(getString("TEXT22"), 100, getString("FONT"), FONT_SIZE, Constants.C_BLACK, Constants.C_TRANSPARENT, 0, Constants.FMT_LEFT | Constants.FMT_VTOP | Constants.FMT_EXCELMODIFY, 0);
        ooutfile.EndTable("", 100, getString("FONT"), FONT_SIZE, Constants.C_BLACK, Constants.C_TRANSPARENT, 0, Constants.FMT_LEFT | Constants.FMT_VTOP, 0);
    } else {
        outHeaderFooterNew(ooutfile);
        ooutfile.OutputLn(getString("TEXT22"), getString("FONT"), FONT_SIZE, Constants.C_RED, Constants.C_TRANSPARENT, Constants.FMT_LEFT, 0);
    }
