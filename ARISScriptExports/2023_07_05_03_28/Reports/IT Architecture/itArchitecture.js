...
// (DUMMY) -> All graphics with same size!
    ...
    oPicture.SelectFont(350, p_sFont, Constants.FMT_BOLD);    
...
function getArrowPicture(p_sFont) {
...
// (DUMMY) -> All graphics with same size!
    ...
    oArrow.SelectFont(350, p_sFont, Constants.FMT_BOLD);    
// Arrowline    
    oArrow.SelectPen(Constants.PS_SOLID, 2, cCOLOR_Pen);
...
// Arrowhead
    ...
    oArrow.SelectBrush(cCOLOR_Pen);
    oArrow.Polygon(arrowHead);

    oArrow.SetTextColor(cCOLOR_Pen);
// Years
    var aYears = getYears(); 
    var nCount = 5;
    var nDelta = 1;
...
// Start / End
    oArrow.SelectFont(250, p_sFont, Constants.FMT_BOLD);    
    ...
    for (var i = 0; i < p_oSelApplSystems.length; i++) {
        ...
        for (var j = 0; j < oInterfaceCxns_Out.length; j++) {
            ...
            if (oTargetObjDef != null) {
                writeDataAndProtocol(oApplSystem, oTargetObjDef, oInterfaceCxn, "-->");
            }
        }
        ...
        for (var j = 0; j < oInterfaceCxns_In.length; j++) {    
            ...
            if (oSourceObjDef != null) {
                writeDataAndProtocol(oApplSystem, oSourceObjDef, oInterfaceCxn, "<--");
            }
        }    
    }
