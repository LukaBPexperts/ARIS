...
function main() {
    ...
    for (var i=0; i<oApplSysTypes.length; i++) {
        ...
        // Persons responsible
        var oPersons = getConnectedObjects(oApplSysType, Constants.EDGES_IN, Constants.CT_IS_RESP_1, Constants.OT_PERS);
        outHeading3(getString("TEXT_61"));
        outPersons(oPersons, oApplSysType, sIndex);
...
// Contacts and persons responsible
function outPersons(p_oObjDefs, p_oApplSysOrApplSysType, p_sIndex) {
    var oPersons = ArisData.Unique(p_oObjDefs);
    var oPersonResponsible = null;
    var oPersonSubstitute = null;
    var oAdditionalContacts = new Array();

    var aPersonsWithRoles = new Array();

    for (var i = 0; i < oPersons.length; i++) {
        var oPerson = oPersons[i];

        var aRoles = getRoleArray(oPerson, p_oApplSysOrApplSysType);
        for (var j = 0; j < aRoles.length; j++) {
            aPersonsWithRoles.push(new PERSON_ROLE(oPerson, aRoles[j]))
        }
    }
    aPersonsWithRoles.sort(sortPersonWithRole);

    for (var j = 0; j < aPersonsWithRoles.length; j++) {
        var sRole = null;
        if (aPersonsWithRoles[j].nRole != -1) sRole = aPersonsWithRoles[j].nRole;

        if (sRole == (Constants.AVT_SUPPORT_TYPE_SYS_RESP)) {
            oPersonResponsible = aPersonsWithRoles[j].oPerson;
        } else if (sRole == (Constants.AVT_SUPPORT_TYPE_SUBST_SYS_RESP)) {
            oPersonSubstitute = aPersonsWithRoles[j].oPerson;
        } else {
            oAdditionalContacts.push(aPersonsWithRoles[j].oPerson);
        }
    }

    g_oOutfile.BeginTable(100, COL_TBL_BORDER, Constants.C_TRANSPARENT, Constants.FMT_LEFT, 0);

    if (oPersonResponsible != null) {
        var sName = getAttrValue(oPersonResponsible, Constants.AT_NAME, true);
        var sPhoneNum = getAttrValue(oPersonResponsible, Constants.AT_PHONE_NUM, false);
        var sEMail = getAttrValue(oPersonResponsible, Constants.AT_EMAIL_ADDR, false);
    } else {
        var sName = "—";
        var sPhoneNum = "";
        var sEMail = "";
    }
    outInfo(getString("TEXT_62"), sName, true);
    if (sPhoneNum.length > 0) {
        outInfo("", sPhoneNum, true);
    }
    if (sEMail.length > 0) {
        outInfo("", sEMail, true);
    }

    if (oPersonSubstitute != null) {
        var psName = getAttrValue(oPersonSubstitute, Constants.AT_NAME, true);
        var psPhoneNum = getAttrValue(oPersonSubstitute, Constants.AT_PHONE_NUM, false);
        var psEMail = getAttrValue(oPersonSubstitute, Constants.AT_EMAIL_ADDR, false);
    } else {
        var psName = "—";
        var psPhoneNum = "";
        var psEMail = "";
    }
    outInfo(getString("TEXT_63"), psName, false);
    if (psPhoneNum.length > 0) {
        outInfo("", psPhoneNum, false);
    }
    if (psEMail.length > 0) {
        outInfo("", psEMail, false);
    }
    ...
    if (oOrgUnits.length > 0)
        outOrgUnits(oOrgUnits, p_oApplSysOrApplSysType, p_sIndex);
    else {
        outInfo(getString("TEXT_66"), "—", true);
    }

    if (oAdditionalContacts.length > 0) {
        for (var i = 0; i < oAdditionalContacts.length; i++) {
            var asName = getAttrValue(oAdditionalContacts[i], Constants.AT_NAME, true);
            var asPhoneNum = getAttrValue(oAdditionalContacts[i], Constants.AT_PHONE_NUM, false);
            var asEMail = getAttrValue(oAdditionalContacts[i], Constants.AT_EMAIL_ADDR, false);
            outInfo(getString("TEXT_67"), asName, false);
            if (asPhoneNum.length > 0) {
                outInfo("", asPhoneNum, false);
            }
            if (asEMail.length > 0) {
                outInfo("", asEMail, false);
            }
        }
    } else {
        outInfo(getString("TEXT_67"), "—", false);
    }
    g_oOutfile.EndTable("", 100, getString("FONT"), 10, Constants.C_BLACK, Constants.C_TRANSPARENT, 0, Constants.FMT_LEFT, 0);
}
...
function outOrgUnits(p_oObjDefs, p_oApplSysOrApplSysType) {
    ...
    outInfo(getString("TEXT_66"), sName, true);
}
...
function outInterfaces(p_oApplSystem) {
    ...
    if (aRowData.length > 0) {
        ...
        g_oOutfile.TableRow();
        // System
        g_oOutfile.TableCellF(getString("TEXT_146"), 20, "TBL_HEAD");
        // Direction
        g_oOutfile.TableCellF(getString("TEXT_42"), 10, "TBL_HEAD");
        // External system
        g_oOutfile.TableCellF(getString("TEXT_147"), 20, "TBL_HEAD");
        // Data type
        g_oOutfile.TableCellF(getString("TEXT_148"), 30, "TBL_HEAD");
        // Protocol
        g_oOutfile.TableCellF(getString("TEXT_41"), 20, "TBL_HEAD");

        for (var i = 0; i < aRowData.length; i++) {
            g_oOutfile.TableRow();
            g_oOutfile.TableCellF(getName(aRowData[i].oSource), 20, getStyleColored("TBL_STD", bColoredTableCell));
            g_oOutfile.TableCellF(aRowData[i].sDirection, 10, getStyleColored("TBL_STD", bColoredTableCell));
            g_oOutfile.TableCellF(getName(aRowData[i].oTarget), 20, getStyleColored("TBL_STD", bColoredTableCell));
            g_oOutfile.TableCellF(getDataTypeNamesList(aRowData[i].oData), 30, getStyleColored("TBL_STD", bColoredTableCell));
            g_oOutfile.TableCellF(getName(aRowData[i].oProtocol), 20, getStyleColored("TBL_STD", bColoredTableCell));
