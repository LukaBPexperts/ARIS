...
function main()
{
    var visio = new T_Visio();
    try
    {
        var gStart = ArisData.getSelectedGroups()[0];
        var check = visio.initVisioFile();
        if(check==null)
            return; //user pressed cancel

        if(check==true){
            var nModelType = visio.getNewModelType();
            nModelType = postInitStep(visio, visio.getMapping().getSourceType(), nModelType); //interceptor defined in 02 user_defined_steps.js
            if(nModelType>-1){
                // if(mNew!=null && mNew.IsValid()){
                    if(!visio.processModelGeneration(gStart,nModelType))
                        Context.setScriptError(Constants.ERR_CANCEL)
                // }
                Context.addActionResult(Constants.ACTION_UPDATE,"",gStart);
            }else{
                Dialogs.MsgBox(getString("FIND_MODELTYPE.ERR"))
            }
        }
        else
            Dialogs.MsgBox(getString("GENERAL_READ_ERROR.ERR"))
    }
    catch(ex)
    {
       ...
       Dialogs.MsgBox(getString("GENERAL_IMPORT_ERROR.ERR")+"\n"+message+"\nin "+filename+" ("+line+")")
       Context.setScriptError(Constants.ERR_CANCEL)
    }
    visio.destroyLoader()
}
...
function visioDialog(p_visioMappings, p_visioPages) {
    ...
    this.onClose = function(pageNumber, bOk) { _closedByOk = bOk}
...
function T_Visio(){
    ...
    this.initVisioFile = function(){
        var bInit = false;
        var data = Context.getProperty("vdx-file")
        if(data==null)
        {
            var sFile = Dialogs.getFilePath(null, getString("ID_FILETYPE.DBI"), null, getString("SELECT_FILETYPE.DBT"), 0);
            if(sFile!=null && sFile.length==1){
                bInit = initVisioData(sFile[0].getData(), sFile[0].getName())
            }
            else
        ...
        {
            bInit = initVisioData(data, Context.getProperty("vdx-filename"))
	    ...
	    function initVisioData(data, name)
	    {
            Context.writeStatus(formatstring1(getString("STATUS_READ.INF"), name), 2)
	        var bInit = false;
	        try{
                ...
                _sUsedTemplate = initTablesAndTemplate(_mapping, sTemplateName)
                ...
                {
                    Dialogs.MsgBox( formatstring1(getString("INCORRECT_VIS_FILE_VERSION.ERR"), _fVisioName) )
                    return false;
                }
                ...
                if(_mapping.initMapping(_sUsedTemplate))
                {
                    ...
                    Dialogs.MsgBox(getString("MAPPING_READ_ERROR.ERR"))
	        }catch(e){
    ...
    function initTablesAndTemplate(_mapping, sTemplateName)
    {
        ...
        return showSelectionDialog(mapIDsToNames, aPages, sCurrentTemplate);
    }

    function showSelectionDialog(p_mappingIDsToNames, p_aVisioPages, p_sTemplateName )
    {
        ...
        {
            var result = Dialogs.showDialog(new visioDialog(p_mappingIDsToNames, p_aVisioPages), Constants.DIALOG_TYPE_ACTION, getString("DIALOG_SELECT_INPUT_SETTINGS.DBT"));
                    ...
                    if(conn.src!=null && conn.trg!=null)
                    {
                        ...
                        if(type>-1){
                            ...
                            if(ccCreated!=null)
                            {
                                ...
                                {
                                    if( conn.properties.containsKey(cxnPropertyList[iCxnAttr][0]) )
                                    {
                        ...
                        if(!bCreated){ // Fallback, if connection in this direction is not allowed the other direction is tried.
                            ...
                            if(type>-1){
                                ...
                                if(ccCreated!=null)
                                {
                                    ...
                                    if(conn.text.length > 0)
                                    {
                                        ccCreated.CxnDef().Attribute(cxnPropertyList[1], g_nLoc, false).setValue(conn.text);
                                        if(!ccCreated.AttrOcc(cxnPropertyList[1]).Exist())
                                            ccCreated.AttrOcc(cxnPropertyList[1]).Create(cxnPropertyList[2], ArisData.getActiveDatabase().defaultFontStyle() )
                                    }
                                    bCreated = true;
                                }
                            }
    ...
    * @param {org.jdom.Element[]} connects List of objects of type org.jdom.Element
    ...
    */
    ...
    function VIS_ShapeToProcess(p_loader){
        ...
        {
            this.vShape = vShape;
            this.properties = m_loader.getShapeProperties(vShape);//pre10.0.4 version should use: thafixed_getShapeProperties(vShape)
        }
    }
    ...
    function VIS_Connection(){
        ...
        this.initPoints = function(tVisio, visPointOfReference, sConnId, vPageOrShape, elConnect)
        {
            ...
            if(elShapeV!=null)
            {
                ...
                this.properties =  tVisio.getLoader().getShapeProperties(elShapeV);//pre10.0.4 version should use: thafixed_getShapeProperties(elShapeV);
                return true
            }//elShapeV!=null
        }
        return false;
...
function getShapeText(tVisio, vShape)
{
    ...
    * @param {org.jdom.Element} elPage page-element of which shapes are searched.
                    ...
                    // try{
                        ...
                        { // search underlying shape elements
                            ...
                            try {
                                ...
                                processShapeTreeBeneath(mModel,vShape,mapReferenceShape.get(nMasterID),visPointOfReference,shapeS,tVisio);
                            }
                            catch(e)
                            {
    ...
    */
    function createVisualShape(tVisio, mModel, shapeToProcess, mapReferenceShape, visPointOfReference, aAggregatedTexts)
    {
        ...
        // createOcc
        var ocShape = createArisObject(tVisio, mModel, shapeToProcess.nSymbol, shapeToProcess.vShape, visPointOfReference, shapeToProcess.properties);
    ...
    * @param {org.jdom.Element} elPage actual Page-Element.
    * @returns {}
    */
    function processShapeTreeBeneath(mModel,vShapeTopLevel,visMasterInfo,visPointOfReference,shapesToAdd, tVisio){
        var javaInt = new java.lang.Integer(0);
        ...
        for(var itr in shapes){
            // try{
                ...
                if(sMasterShapeID!=null){
                    ...
                    if(nSymNum>-1){
                        var ocNew = createArisObject(tVisio, mModel,nSymNum,vShape,visPointOfReference,null);
                    ...
                    else
                    {
                        ...
                        {
                            result =processShapeTreeBeneath(mModel,vShape,visMasterInfo,visPointOfReference,shapesToAdd,tVisio)
                        }
    ...
    function createArisObject(tVisio, mNew,nSymNum,vShape,visPointOfReference, shapeProperties){
        ...
        if(objtype!=null && objtype>-1)
        {
            ...
            if(odShape!=null && odShape.IsValid()){
                ...
                var propertyIterator = shapeProperties.entrySet().iterator()
                while(propertyIterator.hasNext())
                {
                    var property = propertyIterator.next()
                    var nATN = getObjectAttributeMapping(property.getKey())
                    if(nATN>0)
                    {
                        try{
                            odShape.Attribute(nATN, g_nLoc).setValue(property.getValue())
                        } catch(e) {;}
                    }
                    else
                    {
                        Context.writeLog("Visio object property not mapped: "+property.getKey() + ". For object: "+sName)
                    }
                }
        ...
        function getObjectAttributeMapping(sKey)
        {
    ...
    /**
    * @param {org.jdom.Element} vGroupShape shape-element with attribute "Type" is "Group"
    * @returns {org.jdom.Element[]} List of child elements of elGroupShape where attribut Master
